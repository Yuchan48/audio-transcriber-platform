...
# import schema
from app.schemas.audio import AdminAudioFileResponse, AudioFileCreate, AudioFileMetadata
...
@router.get("/", response_model=list[AudioFileMetadata])
def list_audio_files(
    ...
    return [
        AudioFileMetadata(id=f.id, filename=f.filename, status=f.status)
...
def get_audio_file(audio_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if audio_file.status != "completed":
        raise HTTPException(
            status_code=400, detail="Audio file is not ready for download"
        )
...
def get_transcription(audio_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    audio_file = (
        db.query(AudioFile)
        .options(joinedload(AudioFile.transcriptions))
    ...
    if audio_file.user_id != user_id and role != "admin":
        raise HTTPException(
            status_code=403, detail="Not authorized to access this transcription"
        )
...
@router.get("/all", response_model=list[AdminAudioFileResponse])
