    ...
    try:
        ...
        with open(file_path, "rb") as audio_file:
            response = requests.post(
                DEEPGRAM_ENDPOINT, headers=headers, params=params, data=audio_file
            )
        if response.status_code != 200:
            raise Exception(
                f"Deepgram API request failed with status code {response.status_code}: {response.text}"
            )
        data = response.json()
        return data["results"]["channels"][0]["alternatives"][0]["transcript"]
    except Exception as e:
        print(f"Error in transcribing file {file_path}: {str(e)}")
        raise
